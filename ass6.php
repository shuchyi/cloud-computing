...
// close connection
                ...
                <div class="col-11 p-0 ps-2 pt-2">
                    ...
                    $sql = "SELECT msgid, msgtitle, msgcontent,studentemail,studentemailre FROM msg WHERE studentemail = '$studentemail' AND status = 'archive'";
                    $result = mysqli_query($conn, $sql);
...
// check if there are any results
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<button class='btn btn-lg btn-block' style='background-color: #fdfdfd;' onclick=\"location.href='active_msg.php?msgid=" . $row['msgid'] . "'\">Archive</button>";
